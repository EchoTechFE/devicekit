/**
 * A device's safe-area inset on the status-bar edge is the usable strip beyond
 * the bar — it can never be shallower than the bar itself, or a page would draw
 ...
 * `safeAreaInsetsLandscape` unset must still get an inset on the status-bar edge that matches
...
import { PLATFORM_DEFAULTS, resolveDevice, safeAreaInsetsFor, statusBarEdgeFor, statusBarHeightFor, type DeviceProfile } from './devices.js'
import { DEVICES } from './presets/index.js'

describe('safe-area inset on the status-bar edge vs. status bar height', () => {
  it.each(DEVICES.map((profile) => [profile.name, profile] as const))(
    'never draws %s content under its own status bar edge, portrait or landscape',
    (_name: string, profile: DeviceProfile) => {
      ...
      for (const orientation of ['portrait', 'landscape'] as const) {
        ...
        const edge = statusBarEdgeFor(resolved, orientation)
        expect(inset[edge], `${profile.name} ${orientation} ${edge}`).toBeGreaterThanOrEqual(statusBar)
      }
    },
  )
