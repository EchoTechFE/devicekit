...
import type { DiminaDeviceFrame } from '../device-frame.js'
...
describe('<DeviceFrame>', () => {
  it('renders and registers the underlying custom element', () => {
    ...
    expect(customElements.get('dimina-device-frame')).toBeDefined()
    expect(container!.querySelector('dimina-device-frame')).not.toBeNull()
  })
  ...
  it('maps string/boolean props onto matching attributes, and omits embedded when false', () => {
    ...
    const el = container!.querySelector('dimina-device-frame')!
    ...
    const el2 = container!.querySelector('dimina-device-frame')!
  ...
  it('sets deviceProfile as a property, updates it on rerender, and clears it when passed null', () => {
    ...
    const el = container!.querySelector('dimina-device-frame') as DiminaDeviceFrame
  ...
  it('forwards a ref to the DiminaDeviceFrame instance', () => {
    const ref = createRef<DiminaDeviceFrame>()
    mount(<DeviceFrame device="iPhone 15" ref={ref} />)
    expect(ref.current).not.toBeNull()
    expect(ref.current!.tagName.toLowerCase()).toBe('dimina-device-frame')
    expect(ref.current).toBe(container!.querySelector('dimina-device-frame'))
  })
