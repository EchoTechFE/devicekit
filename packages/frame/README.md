...
| `interaction-mode` | `mobile`, `mobile-no-touch`, `desktop`, `desktop-touch` | Defaults to `mobile`; controls the preview cursor and publishes the resolved mode to slotted content |
...
`interaction-mode` makes a desktop preview read as mobile or desktop input. `mobile` and `desktop-touch` show the centered touch cursor; `mobile-no-touch` and `desktop` show the normal cursor. The resolved value is available as `frame.interactionMode` and `data-devicekit-interaction-mode` on the frame, so slotted content can switch its own affordances.
...
| `interactionMode` | `DeviceInteractionMode` | Reflects `interaction-mode`; unsupported values resolve to `mobile` |
